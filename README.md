...
- `NEXT_PUBLIC_API_BASE_URL`, `API_BASE_URL` — URL where the PHP API is served (defaults to `http://localhost:8080`).
...
### 4. Run the PHP API
Serve the API directory with PHP's built-in server (or your preferred web server):
```bash
php -S 0.0.0.0:8080 -t api
```

Ensure the host/port matches the values in `.env` (`API_BASE_URL` & `NEXT_PUBLIC_API_BASE_URL`).
